...
export function validatePublicOrder(payload: Record<string, unknown>) {
  ...
  if (thana.length < 2 || !/[\p{L}\p{N}]/u.test(thana)) return failure("Enter a valid thana or upazila.");
  ...
  return {
    ok: true as const,
    order: { customerName, phone, quantity, district, thana, address, note } satisfies PublicOrder,
  };
}
