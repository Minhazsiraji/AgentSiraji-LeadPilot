...
import { whatsappMessageUrl } from "../lib/order-confirmation";
...
export default function LeadPilotApp({ initialNow, user }: { initialNow: string; user: ChatGPTUser | null }) {
  ...
  const filteredLeads = useMemo(() => {
    ...
    return leads.filter((lead) => {
      const needsAttention = ["Reply Approval", "Confirmation Approval", "Needs Review", "Needs Reply", "Follow-up Due", "Spam"].includes(lead.attentionState);
      const matchesTab =
        activeTab === "All leads" ||
      ...
      <section className="workspace" id="top">
        <header className="utility-bar">
          ...
          <div className="utility-actions">
            {!isDemo ? <div className="notification-wrap"><button aria-expanded={notificationsOpen} aria-label={`${unreadNotifications.length} unread order notifications`} className="notification-button" onClick={() => setNotificationsOpen((open) => !open)} type="button">🔔{unreadNotifications.length ? <span>{unreadNotifications.length}</span> : null}</button>{notificationsOpen ? <div className="notification-panel"><div className="notification-heading"><strong>Order notifications</strong>{unreadNotifications.length ? <button onClick={() => void markAllNotificationsRead()} type="button">Mark all read</button> : null}</div>{notifications.length ? notifications.slice(0, 10).map((item) => <button className={item.readAt ? "notification-item" : "notification-item is-unread"} key={item.id} onClick={() => void openNotification(item)} type="button"><strong>{item.title}</strong><span>{item.message}</span><small>{relativeTime(item.createdAt, relativeTimeNow)}</small></button>) : <p className="notification-empty">New verified orders will appear here.</p>}</div> : null}</div> : null}
        ...
        <div className="page-content">
          ...
          <section className="inbox-card" aria-labelledby="lead-inbox-title">
            ...
            <div className="tabs" role="tablist" aria-label="Lead filters">
              {["Needs attention", "All leads", ...pipelineStages.slice(0, 5)].map((tab) => <button aria-selected={activeTab === tab} className={activeTab === tab ? "tab is-active" : "tab"} key={tab} onClick={() => setActiveTab(tab)} role="tab" type="button">{tab}{tab === "Needs attention" ? <span>{leads.filter((lead) => ["Reply Approval", "Confirmation Approval", "Needs Review", "Needs Reply", "Follow-up Due", "Spam"].includes(lead.attentionState)).length}</span> : null}</button>)}
            </div>
              ...
              <table>
                ...
                <tbody>
                  {filteredLeads.map((lead) => {
                    const attention = ["Reply Approval", "Confirmation Approval", "Needs Review", "Needs Reply", "Follow-up Due", "Spam"].includes(lead.attentionState);
...
function LeadDrawer({ lead, currency, offeringLabel, onClose, onChanged }: { lead: PreviewLead; currency: string; offeringLabel: string; onClose: () => void; onChanged: (message: string) => Promise<void> }) {
  ...
  const isOrderConfirmation = lead.draft?.draftType === "order_confirmation";
  const whatsappUrl = isOrderConfirmation ? whatsappMessageUrl(lead.phone, draft) : null;
  ...
  return <div className="drawer-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><aside aria-labelledby="lead-detail-title" aria-modal="true" className="lead-drawer" role="dialog"><header className="drawer-header"><div><p className="eyebrow">{lead.temperature} · {lead.leadScore}/100</p><h2 id="lead-detail-title">{lead.customerName}</h2><p>{lead.attentionState}</p></div><button aria-label="Close lead details" onClick={onClose} type="button">×</button></header><div className="drawer-scroll">
    ...
    {lead.draft ? <section className={isOrderConfirmation ? "detail-section confirmation-section" : "detail-section"}><div className="detail-section-heading"><div><p className="detail-label">{isOrderConfirmation ? "Customer message" : "Reply draft"}</p><h3>{isOrderConfirmation ? "Order confirmation" : lead.draft.approvalStatus === "approved" ? "Approved response" : "Owner approval required"}</h3></div><span className={`approval-pill approval-${lead.draft.approvalStatus}`}>{lead.draft.approvalStatus === "approved" ? "sent" : "ready to send"}</span></div>{isOrderConfirmation ? <p className="confirmation-note">Prepared automatically from the verified order. Review it before sending.</p> : null}<textarea className="draft-editor" onChange={(event) => setDraft(event.target.value)} rows={isOrderConfirmation ? 13 : 10} value={draft} /><div className="inline-actions">{isOrderConfirmation && whatsappUrl && lead.draft.approvalStatus !== "approved" ? <a className="button button-whatsapp" href={whatsappUrl} rel="noreferrer" target="_blank">Open WhatsApp</a> : null}<button className="button button-quiet" onClick={() => void navigator.clipboard.writeText(draft)} type="button">{isOrderConfirmation ? "Copy message" : "Copy reply"}</button><button className="button button-primary" disabled={busy || lead.draft.approvalStatus === "approved" || lead.doNotContact || lead.possibleSpam} onClick={() => void action(`/api/leads/${lead.id}/approve`, { method: "POST", body: JSON.stringify({ message: draft }) }, isOrderConfirmation ? "Order confirmation recorded as sent." : "Reply approved, contact recorded, and follow-up activated.")} type="button">{isOrderConfirmation ? "Mark as sent" : "Approve & record contact"}</button></div></section> : <section className="detail-section warning-section"><strong>No reply was created.</strong><span>This lead is spam, Do Not Contact, or needs manual review.</span></section>}
